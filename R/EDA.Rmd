...
## Trim to top 10% of species, lumping everything else into 'other'  

```{r}
trimmed <- cover_long %>% 
    filter(!(species %in% to_exclude)) %>% 
    mutate(species = fct_other(species, keep = c(ten_pct$species)))
```

Now need to add up all those "Other"s for each site/plot/date.  
...
For Great Bay, this reduced the number of rows from 230,000 to 19,000. That's pretty impressive. A lot of the species are still showing 0s at Bunker Creek (probably not at other sites).  

```{r}
# ggplot(trimmed2) +
#     geom_point(aes(x = year, y = pct_cover, color = as.factor(transect_number)), alpha = 0.5, size = 2) +
#     facet_grid(species ~ site)
```

```{r}
filter(trimmed2, site == "Bunker Creek") %>% 
    ggplot() +
    geom_point(aes(x = year, y = pct_cover, color = as.factor(transect_number)), alpha = 0.5, size = 2) +
    facet_wrap(~species, ncol = 4)
```

Generalize that into a function  

```{r}
plot_one_site <- function(data, site, ncol){
    dat <- data[, site == site]
    ggplot(dat) +
...
for(i in seq_along(all_sites)){
    print(plot_one_site(trimmed2, all_sites[i], 3))
}
```

# TROUBLESHOOT ME  

Something's wrong..... all the graphs coming out of the loop are the same.  
